...
DHS_DATA_DIR = Path(str(BASE_DIR / "__data__" / "dhs"))
CONJUNGTIVA_DATA_DIR = Path(str(BASE_DIR / "__data__" / "ghanna_conjungtiva"))

DHS_REPO = "Gamma-Fest-2026/dhs-dataset"
CONJUNGTIVA_REPO = "Gamma-Fest-2026/mendeley-conjungtiva"
...
def setup_session():
    ...
    print("  Select HuggingFace Repository:")
    print(f"  1. DHS Repo        ({DHS_REPO})")
    print(f"  2. Conjungtiva Repo ({CONJUNGTIVA_REPO})")
    print("  3. Custom Repo ID")

    while True:
        repo_choice = input("  Choice [1-2]: ").strip()

        if repo_choice == "1":
            HF_REPO_ID = DHS_REPO
            break
        elif repo_choice == "2":
            HF_REPO_ID = CONJUNGTIVA_REPO
            break
        elif repo_choice == "3":
            while True:
                custom_repo = input("  Enter custom Repo ID (e.g., username/repo): ").strip()
                if custom_repo:
                    HF_REPO_ID = custom_repo
                    break
                print("  [!] Error: Repo ID cannot be empty.")
            break
        else:
            print("  [!] Error: Invalid selection. Please enter 1 or 2.")

    print("\n  Select local data directory:")
    print(f"  1. DHS Dataset     ({DHS_DATA_DIR})")
    print(f"  2. Conjungtiva     ({CONJUNGTIVA_DATA_DIR})")
    print("  3. Custom Path")

    while True:
        dir_choice = input("  Choice [1-2]: ").strip()

        if dir_choice == "1":
            DATA_DIR = DHS_DATA_DIR
            break
        elif dir_choice == "2":
            DATA_DIR = CONJUNGTIVA_DATA_DIR
            break
        elif dir_choice == "3":
            while True:
